...
struct ibv_mr *create_ibv_mr(struct ibv_pd *pd) {
    ...
    struct ibv_mr *mr = ibv_reg_mr(pd, mr_addr, MR_SIZE, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE);
...
void send_qp_data(socket_t sock, struct client_conn_data_s *conn_data) {
    int rc = send(sock, conn_data, sizeof(struct client_conn_data_s), MSG_WAITALL);
    if (rc == SOCKET_ERROR) {
        perror("send()");
        exit(EXIT_FAILURE);
    }
...
struct connection_data_s recv_qp_data(socket_t sock) {
    ...
    int rc = recv(sock, &remote_data, sizeof(remote_data), MSG_WAITALL);
    check_error(rc, "recv()");
...
struct ib_resources_s *connect_ib_server(struct ib_handle_s *ib_handle) {
    ...
    struct client_conn_data_s client_conn_data = (struct client_conn_data_s) {
        .conn_data = {
    ...
    send_qp_data(ib_res->sock, &client_conn_data);
    struct connection_data_s conn_data = recv_qp_data(ib_res->sock);
    modify_qp_to_init(ib_res->qp);
    modify_qp_to_rtr(ib_res->qp, conn_data);
...
void post_send(struct ibv_qp *qp, struct ibv_mr *mr, int req_size) {
    struct ibv_sge sge;
...
void poll_completion(struct ib_handle_s *ib_handle, int count) {
    ...
    do {
        rc = ibv_poll_cq(ib_handle->cq, count, wc);
        if (rc < 0) {
            perror("ibv_poll_cq");
            exit(EXIT_FAILURE);
        }
