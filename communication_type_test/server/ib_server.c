...
void poll_completion(struct ib_server_resources_s *res) {
    ...
    do {
        ...
        for (int i = 0; i < rc; i++) { 
            ...
            struct ibv_qp *qp = (struct ibv_qp *)get(res->qp_map, wc[i].qp_num);
            struct ibv_mr *mr = (struct ibv_mr *)wc[i].wr_id;
            struct message_header_s *header = (struct message_header_s *)mr->addr;

            if (header->type == TYPE_1) {
                switch (wc[i].opcode) {
                    case IBV_WC_RECV:
                        post_send(qp, mr, sizeof(struct message_header_s) + header->data_size);
                        break;
            ...
            else if(header->type == TYPE_2) {
                switch (wc[i].opcode) {
                    case IBV_WC_RECV:
                        post_write(qp, mr, header);
                        break;
                    
                    case IBV_WC_RDMA_WRITE:
                        post_send(qp, mr, sizeof(struct message_header_s));
                        break;
            ...
            else if(header->type == TYPE_3) {
                switch (wc[i].opcode) {
                    case IBV_WC_RECV:
                        post_read(qp, mr, header);
                        break;
